...
const CampaignCard = ({ campaign }) => {
  return (
    <div className="bg-white rounded-2xl p-6 lg:p-8 shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-100 flex flex-col w-full lg:w-[668px] max-w-full lg:max-w-xl mx-auto lg:mx-0 relative z-10">
      {/* Header & Icon */}
      <div className="flex items-center gap-3 lg:gap-4 mb-4">
        <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl bg-[#2a2a2a] flex items-center justify-center shrink-0">
          <div className="w-4 h-4 lg:w-5 lg:h-5 border-2 border-white rounded-sm"></div>
        </div>
        <h3 className="text-[22px] md:text-[24px] lg:text-[28px] font-semibold text-gray-900 leading-tight">
          {campaign.title}
        </h3>
      </div>

      {/* Description */}
      <p className="text-gray-600 text-[14px] lg:text-[15px] leading-relaxed mb-6 lg:mb-8 pr-0 lg:pr-4">
        {campaign.description}
      </p>
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          <span className="text-[12px] lg:text-[13px] font-semibold text-gray-900">
            {campaign.metricLabel}
          </span>
          <span className="text-[12px] lg:text-[13px] font-semibold text-gray-900">
            {campaign.metricPercentage}% of Goal
        ...
        {/* Progress Bar */}
        <div className="w-full h-2 lg:h-2.5 bg-gray-200 rounded-full overflow-hidden mb-3">
          <div
            className="h-full rounded-full transition-all duration-500 ease-out"
            style={{
              width: `${campaign.metricPercentage}%`,
      ...
      {/* Button */}
      <button
        className="w-full py-3 lg:py-3.5 rounded-xl font-semibold text-gray-900 text-[15px] lg:text-[17px] transition-opacity hover:opacity-90"
        style={{ backgroundColor: campaign.themeColor }}
      >
        {campaign.buttonText}
      </button>
...
const CampaignsSection = () => {
  return (
    <section className="relative w-full min-h-screen bg-[#FBFAF1] overflow-hidden py-12 lg:py-24 font-sans">
      {/* BACKGROUND IMAGE */}
      <img
        src={causeArtwork}
        alt=""
        className="absolute top-0 left-0 w-full h-[400px] lg:h-[600px] object-cover object-top opacity-100 pointer-events-none z-0"
        aria-hidden="true"
      />

      <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-[40px] relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8">
        {/* Left Column */}

        {/* Left Column */}
        <div className="lg:sticky lg:top-24 h-fit flex flex-col justify-start pt-4 lg:pt-8">
          <div className="max-w-md">
            {/* 
              FIXED HEADING: 
              - Split into spans for 'Satisfy' and 'Sora' fonts.
              - Mobile size set to exactly 32px (per Figma).
              - Desktop locked to 56px.
              - <br /> now applies on all screens.
            */}
            <h2 className="mb-4 lg:mb-6 text-gray-900 m-0">
              <span className="font-['Satisfy'] text-[32px] md:text-[40px] lg:text-[56px] font-normal leading-[125%] lg:leading-[1.1] tracking-[-0.02em]">
                Join A Cause
              </span>
              <span className="font-['Sora'] text-[32px] md:text-[40px] lg:text-[56px] font-normal leading-[125%] lg:leading-[1.1] tracking-[-0.03em]">
                {" "}
                That's <br /> Live Right Now
              </span>
            </h2>

            <p className="text-[14px] lg:text-[15px] text-gray-800 leading-relaxed max-w-sm mt-4 lg:mt-0">
              Support active campaigns that provide education, nutrition, and
              opportunities for children who need them most.
            </p>
          </div>
          ...
          {/* FOREGROUND IMAGE */}
          <div className="mt-8 lg:mt-12 w-full max-w-[600px] ml-0 lg:-ml-4">
            <img
              src={kidsGroup}
              alt="Children reading and eating together"
              className="w-full h-auto object-contain relative z-10"
        ...
        {/* Right Column */}
        <div className="flex flex-col gap-6 lg:gap-[30px] items-center lg:items-end">
          {campaigns.map((campaign) => (
            <CampaignCard key={campaign.id} campaign={campaign} />
          ))}
        </div>
